// 完整拼豆色板数据 - 291色 × 5品牌
// 数据来源: perler-beads 开源项目 colorSystemMapping.json

const COLOR_SYSTEM_MAPPING = {
...
// 品牌选项
export const COLOR_SYSTEM_OPTIONS = [
  { key: 'MARD', name: 'MARD' },
  { key: 'COCO', name: 'COCO' },
  { key: '漫漫', name: '漫漫' },
  { key: '盼盼', name: '盼盼' },
...
/**
 * 根据品牌构建色板
 * @param {string} brand - 品牌名称: MARD/COCO/漫漫/盼盼/咪小窝
 * @returns {Array<{key: string, hex: string, rgb: {r:number,g:number,b:number}}>}
 */
export function buildPalette(brand = 'MARD') {
  const palette = []
  for (const [hex, mapping] of Object.entries(COLOR_SYSTEM_MAPPING)) {
    const brandKey = mapping[brand]
    if (!brandKey || brandKey === '-') continue
    const rgb = hexToRgb(hex)
    if (!rgb) continue
    palette.push({ key: hex, hex, rgb, brandKey })
  }
  return palette
}
 ...
 * 获取指定 hex 颜色在某品牌下的色号
 */
export function getColorKey(hex, brand = 'MARD') {
  const normalizedHex = hex.toUpperCase()
  const mapping = COLOR_SYSTEM_MAPPING[normalizedHex]
  if (mapping && mapping[brand]) return mapping[brand]
  return '?'
}
